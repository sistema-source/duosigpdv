...
uses
  Classes, SysUtils, Forms, Graphics, Dialogs, ACBrTEFD,
  ACBrPosPrinter, view_pai, ACBrTEFDClass,
  Controls, ExtCtrls, DateUtils, StdCtrls,
  model_configuracao_inifile,
  ACBrTEFPayGoComum, lib_funcoes,
...
type
  ...
  { TModelPayGoTef }
  TModelPayGoTef = class(TViewPai)
    ACBrPosPrinter: TACBrPosPrinter;
    ACBrTEFD: TACBrTEFD;
    PnlMensagemPaygo: TPanel;
...
procedure TModelPayGoTef.ACBrTEFDAguardaResp(Arquivo: string;
...
procedure TModelPayGoTef.ACBrTEFDAntesFinalizarRequisicao(Req: TACBrTEFDReq);
begin
...
procedure TModelPayGoTef.ACBrTEFDBloqueiaMouseTeclado(Bloqueia: boolean;
  var Tratado: boolean);
begin
...
procedure TModelPayGoTef.ACBrTEFDComandaECF(Operacao: TACBrTEFDOperacaoECF;
  Resp: TACBrTEFDResp; var RetornoECF: integer);
begin
...
procedure TModelPayGoTef.ACBrTEFDComandaECFAbreVinculado(COO, IndiceECF: string;
  Valor: double; var RetornoECF: integer);
begin
  RetornoECF := 1;
...
procedure TModelPayGoTef.ACBrTEFDComandaECFImprimeVia(TipoRelatorio:
  TACBrTEFDTipoRelatorio; Via: integer; ImagemComprovante: TStringList;
  var RetornoECF: integer);
var
  LDocImpresso: string;
begin
  PnlMensagemPaygo.Caption := 'Imprimir a via ' + IntToStr(Via);
...
procedure TModelPayGoTef.ACBrTEFDComandaECFPagamento(IndiceECF: string;
  Valor: double; var RetornoECF: integer);
begin
...
procedure TModelPayGoTef.ACBrTEFDDepoisConfirmarTransacoes(
...
procedure TModelPayGoTef.ACBrTEFDExibeMsg(Operacao: TACBrTEFDOperacaoMensagem;
  Mensagem: string; var AModalResult: TModalResult);
var
  Fim: TDateTime;
  OldMensagem: string;
...
procedure TModelPayGoTef.ACBrTEFDGravarLog(const GP: TACBrTEFDTipo;
...
procedure TModelPayGoTef.ACBrTEFDInfoECF(Operacao: TACBrTEFDInfoECF;
...
procedure TModelPayGoTef.SetBandeira(AValue: string);
begin
  if FBandeira = AValue then Exit;
  FBandeira := AValue;
end;

procedure TModelPayGoTef.SetCnpjAdquirente(AValue: string);
begin
  if FCnpjAdquirente = AValue then Exit;
  FCnpjAdquirente := AValue;
end;

procedure TModelPayGoTef.SetCodAutorizacao(AValue: string);
begin
  if FCodAutorizacao = AValue then Exit;
  FCodAutorizacao := AValue;
end;

procedure TModelPayGoTef.SetDoc001(AValue: string);
begin
  if FDoc001 = AValue then Exit;
  FDoc001 := AValue;
end;

procedure TModelPayGoTef.SetDoc002(AValue: string);
begin
...
procedure TModelPayGoTef.SetIdTransacao(AValue: string);
begin
  if FIdTransacao = AValue then Exit;
  FIdTransacao := AValue;
end;

procedure TModelPayGoTef.SetMensagemFoiMostrada(AValue: boolean);
begin
  if FMensagemFoiMostrada = AValue then Exit;
  FMensagemFoiMostrada := AValue;
end;

procedure TModelPayGoTef.SetMmLog(AValue: TMemo);
begin
  if FMmLog = AValue then Exit;
  FMmLog := AValue;
end;

procedure TModelPayGoTef.SetModeloFiscal(AValue: string);
begin
  if FModeloFiscal = AValue then Exit;
  FModeloFiscal := AValue;
end;

procedure TModelPayGoTef.SetNSU(AValue: string);
begin
  if FNSU = AValue then Exit;
  FNSU := AValue;
end;

procedure TModelPayGoTef.SetPnlMsgCliente(AValue: TPanel);
begin
  if FPnlMsgCliente = AValue then Exit;
  FPnlMsgCliente := AValue;
end;

procedure TModelPayGoTef.SetPnlMsgOperador(AValue: TPanel);
begin
  if FPnlMsgOperador = AValue then Exit;
  FPnlMsgOperador := AValue;
end;

procedure TModelPayGoTef.SetPrimeiraVia(AValue: string);
begin
  if FPrimeiraVia = AValue then Exit;
  FPrimeiraVia := AValue;
end;

procedure TModelPayGoTef.SetQtdParcelas(AValue: integer);
begin
  if FQtdParcelas = AValue then Exit;
  FQtdParcelas := AValue;
end;

procedure TModelPayGoTef.SetRede(AValue: string);
begin
  if FRede = AValue then Exit;
  FRede := AValue;
end;

procedure TModelPayGoTef.SetVlrVenda(AValue: currency);
begin
  if FVlrVenda = AValue then Exit;
  FVlrVenda := AValue;
end;

procedure TModelPayGoTef.ConfigurarImpressora;
begin
...
procedure TModelPayGoTef.MensagemTEF(const MsgOperador, MsgCliente: string);
begin
  if (MsgOperador <> '') then
  begin
...
function TModelPayGoTef.ObterEnum(pNome: string): integer;
begin
...
function TModelPayGoTef.ObterEnumPaginaCodigo(pNome: string): integer;
begin
...
procedure TModelPayGoTef.ViewLog(pString: string);
begin
  //  showMessage(pString);
end;

procedure TModelPayGoTef.AtivarTef;
begin
...
procedure TModelPayGoTef.ConfigurarTef;
...
procedure TModelPayGoTef.Administrativo;
begin
  try
    FPrimeiraVia := '';
    FModeloFiscal := '99';
...
procedure TModelPayGoTef.ImprimirDoc(pValue: string);
begin
...
function TModelPayGoTef.VenderCartaoCredito(pAdquirente: string;
  pVlrVenda: currency): boolean;

var
  LRede, LNSU, LFinalizacao, LDocumentoVinculado: string;
begin

result := false;
  try
    PnlMensagemPaygo.Caption := '';
    Application.ProcessMessages;
    ...
    begin
      ...
      PnlMensagemPaygo.Caption := 'Imprimir Transacao Pendente';
      Application.ProcessMessages;
...
function TModelPayGoTef.VenderCartaoDebito(pAdquirente: string;
  pVlrVenda: currency): boolean;
begin
  ACBrTEFD.TEFPayGoWeb.ParametrosAdicionais.Clear;
...
procedure TModelPayGoTef.AdicionarLinhaLog(AMensagem: string);
begin
